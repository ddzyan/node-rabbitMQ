...
class Consumer {
   ...
   */
  async _createAndBindQueue(queueName, key, option) {
    if (!queueName) {
  ...
  async createQueue(queueOption, exchangerOption) {
    ...
    const { queueName, key, option } = queueOption;
    ...
    // 声明队列，并且绑定交换机
    await this._createAndBindQueue(queueName, key, option);
  }
